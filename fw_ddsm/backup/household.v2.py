...
class Household:
    ...
    def new(self, num_intervals, scheduling_method,
        ...
        self.tasks, self.household_tracker = \
            new_household(num_intervals=num_intervals,
                          ...
                          preferred_demand_profile_csv=preferred_demand_profile_csv,
                          list_of_devices_power_csv=list_of_devices_power_csv,
                          ...
                          write_to_file_path=write_to_file_path, household_id=household_id)

    def read(self, scheduling_method, read_from_file=None):
        self.tasks, self.household_tracker \
            = read_household(scheduling_method=scheduling_method, household_tracker=self.household_tracker,
                             read_from_file=read_from_file)

    def schedule(self, prices, model=None, solver=None, search=None):
        return schedule_household(prices, scheduling_method=self.scheduling_method, household=self.tasks,
                                  num_intervals=self.num_intervals, model=model, solver=solver, search=search)

    def finalise(self, probability_distribution):
        chosen_demand_profile, chosen_penalty, self.household_final \
            = finalise_schedule(probability_distribution=probability_distribution,
                                household_tracker_data=self.household_tracker.data,
                                scheduling_method=self.scheduling_method, num_schedule=self.num_intervals)
        return chosen_demand_profile, chosen_penalty


def read_household(scheduling_method, household_tracker=Tracker(), read_from_file=None):
    tasks = __existing_household(household_file=read_from_file)
...
def new_household(num_intervals, scheduling_method,
    ...
    tasks, household_demand_profile \
        = __new_tasks(num_intervals=num_intervals,
                      preferred_demand_profile=preferred_demand_profile,
                      list_of_devices_power=list_of_devices_power,
                      max_demand_multiplier=max_demand_multiplier,
                      num_tasks_dependent=num_tasks_dependent,
    ...
    if write_to_file_path is not None:
        __write_to_file(tasks=tasks, write_to_file_path=write_to_file_path, household_id=household_id)
...
def schedule_household(prices, scheduling_method, household, num_intervals=no_intervals,
                       model=None, solver=None, search=None):
    prices = __convert_price(num_intervals, prices)
    ...
    if "minizinc" in scheduling_method:
        ...
        actual_starts, household_demand_profile, time_scheduling \
            = __minizinc_model(model_file=model, solver=solver, search=search,
        ...
        actual_starts, household_demand_profile, time_scheduling \
            = __ogsa(objective_values=objective_values, big_value=big_value,
...
def finalise_schedule(probability_distribution,
                      household_tracker_data, household_final=Tracker(), scheduling_method=None, num_schedule=0):
    ...
    if household_final is not None:
        household_final.new(method=scheduling_method)
        household_final.update(num_record=num_schedule, method=scheduling_method,
                               demands=chosen_demand_profile, penalty=chosen_penalty)
        return chosen_demand_profile, chosen_penalty, household_final
    else:
        return chosen_demand_profile, chosen_penalty


def __write_to_file(tasks, write_to_file_path, household_id=0):
...
def __convert_price(num_intervals, prices):
...
def __new_task(num_intervals, num_periods, num_intervals_periods,
               mode_value, list_of_devices_power, pst_probabilities,
               max_care_factor,
    ...
    def get_new_tasks(num_tasks, scheduling_window_width):
        for counter_j in range(num_tasks):
            demand, duration, p_start, e_start, l_finish, care_f \
                = __new_task(num_intervals=num_intervals, num_periods=num_periods,
                             num_intervals_periods=num_intervals_periods,
                             mode_value=mode_value, list_of_devices_power=list_of_devices_power,
                             pst_probabilities=pst_probabilities, max_care_factor=max_care_factor,
                             scheduling_window_width=scheduling_window_width)
...
def __existing_household(household_file):
    # ---------------------------------------------------------------------- #
    # ---------------------------------------------------------------------- #
...
def __minizinc_model(model_file, solver, search,
                     ...
                     care_factors, prices, inconvenience_cost_weight, num_intervals=no_intervals):
    # problem model
...
def __ogsa(objective_values, big_value, powers, durations, preferred_starts, latest_ends, max_demand,
