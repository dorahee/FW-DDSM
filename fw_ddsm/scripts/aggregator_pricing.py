...
def find_step_size(num_iteration, pricing_method, pricing_table, aggregate_demand_profile, aggregate_inconvenience,
                   aggregate_demand_profile_fw_pre, inconvenience_fw_pre, price_fw_pre, cost_fw_pre,
                   min_step_size=min_step, ignore_tiny_step=False, roundup_tiny_step=False, print_steps=False):
    time_begin = time()

    price_fw = price_fw_pre[:]
    cost_fw = cost_fw_pre
    demand_profile_fw = aggregate_demand_profile_fw_pre[:]
    ...
    PAR_fw = PAR_fw_pre
    change_of_PAR = PAR_fw - PAR_fw_pre
    demand_profile_changed = [d_n - d_p for d_n, d_p in zip(aggregate_demand_profile, aggregate_demand_profile_fw_pre)]
    ...
    while gradient < 0 and step_size_final <= 1 and abs(change_of_cost) > min_abs_change_of_cost:
        ...
        for dp, dn, demand_levels_period in \
                zip(aggregate_demand_profile_fw_pre, aggregate_demand_profile, pricing_table[p_demand_table].values()):
        ...
        demand_profile_fw_temp = [d_p + (d_n - d_p) * step_size_incr for d_p, d_n in
                                  zip(aggregate_demand_profile_fw_pre, aggregate_demand_profile)]
        PAR_fw_temp = max(demand_profile_fw_temp) / average(demand_profile_fw_temp)
        ...
        gradient = change_of_inconvenience + change_of_cost + change_of_PAR

        aggregate_demand_profile_fw_pre = demand_profile_fw_temp[:]
        ...
        if gradient < 0 and step_size_final_temp <= 1 and abs(change_of_cost) > min_abs_change_of_cost:
            step_size_final_prev = step_size_final
            ...
            if step_size_final == 1 or step_size_final_prev == step_size_final:
                break

    print(f"{num_iteration}. "
          f"Best step size {round(step_size_final, 6)}, "
          f"{num_itrs} iterations, cost {cost_fw}, gradient {gradient}, "
